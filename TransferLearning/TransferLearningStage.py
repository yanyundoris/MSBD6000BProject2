...
def Parse_train_val_file(file_dir):
    """Get training and validation image path"""
...
def Convert_prediction_to_array(prediction_DF, test_file, output_name = "project2_20384933.txt"):
    """ Sort and Convert prediction result to array"""
    ...
    np.savetxt(output_name,test_file['data'].values,fmt='%1.0f')

    print(test_file)


def get_prediction(output_filename ='transfer_learning_predict'):

    train_dict = Parse_train_val_file('train.txt')
    val_dict = Parse_train_val_file('val.txt')
    ...
    if not img_list:
        ...
        prediction_result.to_csv(output_filename, sep=' ', index=False)
